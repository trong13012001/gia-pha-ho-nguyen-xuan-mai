...
import { authFormSchema, loginSchema } from "@/schemas/auth.schema";
...
export default function LoginPage() {
    ...
    useEffect(() => {
        setValue("mode", isLogin ? "login" : "register");
    }, [isLogin, setValue]);
    ...
    const handleAuthSubmit = async (values: AuthFormValues) => {
        ...
        try {
            if (values.mode === "login") {
                ...
                const { session, error } = await signInMutation.mutateAsync({
                    email: parsed.email,
                ...
                if (error) {
                    ...
                    if (data.session) {
                        ...
                        const { session: signInSession, error: signInError } =
                        ...
                        if (!signInError && signInSession) {
                            setAuth({
                                userId: signInSession.user.id,
                                email: signInSession.user.email ?? values.email,
                                accessToken: signInSession.access_token,
                            });
                            router.push("/dashboard");
                            router.refresh();
                        } else {
                        ...
                        <div className="flex flex-col gap-4 pt-4">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    setSuccessMessage(null);
                                    reset({
                                        mode: nextIsLogin ? "login" : "register",
